...
public class PermissionManager {
    ...
    public static TPermissionType checkPermission(@NonNull TContextWrap contextWrap, @NonNull Method method) {
        ...
        boolean cameraGranted = true, storageGranted =
                ContextCompat.checkSelfPermission(contextWrap.getActivity(), TPermission.STORAGE.stringValue()) == PackageManager.PERMISSION_GRANTED ? true : false;
    ...
    public static void handlePermissionsResult(Activity activity, TPermissionType type, InvokeParam invokeParam,
        ...
        switch (type) {
            case DENIED:
                listener.takeFail(null, TExceptionType.TYPE_COMPRESS_FAIL);
                break;
            case ONLY_CAMERA_DENIED:
                listener.takeFail(null, TExceptionType.TYPE_NO_CAMERA);
                break;
            case ONLY_STORAGE_DENIED:
                listener.takeFail(null, TExceptionType.TYPE_NO_STORAGE_PERMISSION);
                break;
            case GRANTED:
                try {
